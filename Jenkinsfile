        ...
        stage('Post-Build: Smoke Tests') {
            steps {
                ...
                sh '''
                    ...
                    echo "Nginx Frontend HTTP Status: \$STATUS_CODE"
                    if [ "\$STATUS_CODE" -ne 200 ]; then
                        echo "ОШИБКА: Фронтенд на порту 8081 вернул код \$STATUS_CODE вместо 200!"
                        exit 1
                    fi
                '''
                ...
                sh '''
                    API_CODE=\$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8081/api/health)
                    echo "Nginx Proxy to Backend API HTTP Status: \$API_CODE"
                    if [ "\$API_CODE" -ne 200 ]; then
                        echo "ОШИБКА: API бэкенда недоступен через прокси Nginx (Код: \$API_CODE)!"
                        exit 1
                    fi
                '''
